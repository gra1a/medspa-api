...
def test_appointment_create_naive_future_datetime_treated_as_utc():
    ...
    data = AppointmentCreate(start_time=naive_future, service_ulids=["01ARZ3NDEKTSV4RRFFQ69G5FAV"])
    assert data.start_time.tzinfo is not None
    assert data.service_ulids == ["01ARZ3NDEKTSV4RRFFQ69G5FAV"]
...
def test_appointment_create_naive_past_raises():
    ...
    with pytest.raises(ValueError, match="start_time cannot be in the past"):
        AppointmentCreate(start_time=naive_past, service_ulids=["01ARZ3NDEKTSV4RRFFQ69G5FAV"])
